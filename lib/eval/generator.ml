...
let rec add_message_to_mailbox processes value message updated_processes = 
  ...
  | [] -> 
      if updated_processes = [] then
        failwith "No process found with the given interface name"
      else
...
let rec execute (program,pid,steps,mailbox,comp,env,stack) =
  ...
  if steps >= step_limit then
    ...
    | Seq (comp1, comp2), env, stack ->
      let (status, (_,_,steps',mailbox',_,_,_)) = execute (program,pid, steps+1,mailbox,comp1, env, stack) in
      (match status with
      | Finished | Unfinished ->
          (execute (program,pid, steps', mailbox,comp2, env, stack))
      | Spawned _ | MessageToSend (_, _)->
          (status, (program,pid,steps+1, mailbox',comp2, env, stack)))
  ...
  | (prog, pid, steps, mailbox, comp, env, stack) :: rest ->
      ...
      if steps >= max_steps then begin
        ...
        | MessageToSend (target, message) ->
          ...
          let target_process = add_message_to_mailbox rest target message [] in 
            process_scheduling (updated_process :: target_process) max_steps
