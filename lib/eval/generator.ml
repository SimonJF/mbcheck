...
let rec execute (program,steps,comp,env,stack) =
  Buffer.add_string steps_buffer (print_config (comp,env,stack,steps));
  if steps >= step_limit then
    (Unfinished, (program, steps, comp, env, stack))
  else
    match comp,env,stack with
    | Return _,_,[] -> 
      (Finished, (program, steps+1, comp, env, []))
    
    | Annotate (term, _),env,stack ->
        execute (program, steps+1, term,env,stack)

    | Let {binder; term; cont},env,stack ->
        execute (program, steps+1, term,env,(Frame (binder, cont)) :: stack)
    ...
    | LetPair {binders = (binder1, binder2); pair; cont}, env, stack ->
      ...
      | Pair (v1, v2) ->
          ...
          execute (program, steps+1, cont, env', stack)
      | _ -> failwith_and_print_buffer "Expected a pair in LetPair")
      
    | Seq (comp1, comp2), env, stack ->
      let (status, (_,steps',_,_,_)) = execute (program, steps+1, comp1, env, stack) in
      (match status with
      | Finished | Unfinished ->
          execute (program, steps', comp2, env, stack)
      | Spawned _ ->
          (status, (program, steps+1, comp2, env, stack)))

    | Return v, env, Frame (x, cont) :: stack ->
        let result = eval_of_var env v in
        execute (program, steps+1, cont, ValueEntry ( x, result) :: env, stack) 
      
    | App {func; args}, env, stack -> 
        (match func with
        | Lam {parameters; body; _} -> 
            let new_env = bind_args_paras args parameters @ env in
            execute (program, steps+1, body, new_env, stack)
        | Primitive op -> 
            (match op with
            | "print" ->
                let value_to_print = List.hd (eval_args args env) in
                Buffer.add_string result_buffer (show_value value_to_print);
                execute (program, steps+1, Return (Constant (Unit)), env, stack)
            | "intToString" ->
              let value_to_print = List.hd (eval_args args env) in
                (match value_to_print with
                  | Constant (Int i) ->
                      let converted_string = string_of_int i in
                      execute (program, steps+1, Return (Constant (String converted_string)), env, stack)
              ...
              let _ = Random.self_init () in
                (match value_to_convert with
                  | Constant (Int i) ->
                      ...
                      execute (program, steps+1, Return (Constant (Int random_int)), env, stack)
              ...
              let value2 = List.hd (List.tl list) in
                (match value1, value2 with
                  | Constant (String s1), Constant (String s2) ->
                      ...
                      execute (program, steps+1, Return (Constant (String concatenated_string)), env, stack)
                  | _ -> failwith_and_print_buffer "Expected string arguments for concat primitive")
            | _ ->
              let list = eval_args args env in
              let value1 = List.hd list in
              let value2 = List.hd (List.tl list) in
                execute (program, steps+1, Return (Constant (eval_of_op op value1 value2)), env, stack)
            )
        | Variable (func_var, _) ->
          ...
          | Some func_decl ->
              ...
              let  (_,(_, _, comp_result, _, _)) = execute (program, steps+1,func_decl.decl_body, env', []) in
              execute (program, steps+1,comp_result, env, stack)
    ...
    | If {test; then_expr; else_expr}, env, stack -> 
        ...
        | Constant (Bool true) -> execute (program, steps+1, then_expr, env, stack)
        | Constant (Bool false) -> execute (program, steps+1, else_expr, env, stack)
    ...
    | Case {term; branch1 = (binder1, _), comp1; branch2 = (binder2, _), comp2}, env, stack ->
      ...
      | Inl value ->
          ...
          execute (program, steps+1, comp1, env', stack)
      | Inr value ->
          let env' = ValueEntry ( binder2, value) :: env in
          execute (program, steps+1, comp2, env', stack)
    ...
    | Spawn comp, env, stack ->
      let new_process = (program, 0, comp, env, stack) in
        (Spawned new_process, (program, steps+1, Return (Constant Unit), env, stack))
...
let rec process_scheduling processes max_steps =
  ...
  | [] -> ()
  | (prog, steps, comp, env, stack) :: rest ->
      if steps >= max_steps then
        process_scheduling (rest @ [(prog, 0, comp, env, stack)]) max_steps
      else
        let (execution_status, updated_process) = execute (prog, 0, comp, env, stack) in
          match execution_status with
          | Finished -> process_scheduling rest max_steps
          | Unfinished -> process_scheduling (rest @ [updated_process]) max_steps
          | Spawned new_process-> process_scheduling (new_process :: rest @ [updated_process]) max_steps
  ...
  let initial_process =
    ...
    | Some (App { func = Variable (func_var, _); args }) ->
        ...
        | Some func_decl ->
            ...
            (program, 0, func_decl.decl_body, env, [])
        | None -> failwith_and_print_buffer ("Function " ^ func_name ^ " not found in prog_decls"))
    | Some comp -> (program, 0, comp, [], [])
    | _ -> (program, 0, Return (Constant Unit), [], [])
  in
